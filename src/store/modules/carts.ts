export default {
	namespaced:true,//namespaced 就是命名空间的意思，后续的操作需要加上命名空间，比如，之前提交mutations的形式是 this.$store.commit('increment') 现在需要变成 this.$store.commit('命名空间/increment')这种形式，而命名空间就是文件的名字，这里其实就是 this.$store.commit('demo/increment') 
	state:{
		list:[
			{
				checked:false,
